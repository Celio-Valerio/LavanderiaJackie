    ...
    public function store(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'fecha_desde' => [
            ...
            function ($attribute, $value, $fail) {
                ...
                if ($value != $hoy) {
                    $fail('La fecha de inicio no es valida');
                }
            },
        ],
        'fecha_hasta' => [
            ...
            'after:fecha_desde',
            function ($attribute, $value, $fail) use ($request) {
                $fechaMaxima = now()->addMonths(12)->format('Y-m-d');
                if ($value > $fechaMaxima) {
                    $fail('La fecha de fin no es valida ');
                }
            },
        ],
    ], [
    ...
    DB::beginTransaction();
    try {
        // Crear el cupón con las fechas validadas
        $cupon = Cupon::create([
            'nombre' => $request->nombre,
            'descripcion' => $request->descripcion,
            'tipo' => $request->tipo,
            'valor' => $request->valor,
            'fecha_desde' => now()->format('Y-m-d'), // Fecha de inicio automática (hoy)
            'fecha_hasta' => $request->fecha_hasta,
            'estado' => 'Activo'
        ]);

        // Asociar los clientes al cupón
