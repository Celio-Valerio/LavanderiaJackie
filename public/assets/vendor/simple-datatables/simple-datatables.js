        ...
        1: [function (require, module, exports) {
            (function (global) {
                (function () {
                    ...
                    const path = window.location.pathname; // Por ejemplo: /clientes o /empleados
                    let type = 'resultados'; // Valor por defecto

                    // Obtener la última parte de la ruta
                    const segments = path.split('/').filter(segment => segment); // Filtra segmentos vacíos
                    const lastSegment = segments[segments.length - 1]; // Obtiene el último segmento

                    // Asignar el tipo basado en el último segmento
                    if (lastSegment) {
                        type = lastSegment; // Usa el último segmento como el tipo (clientes, empleados, etc.)
                    }

                    // Ahora 'type' tendrá el valor correcto
                    ...
                    const et = {
                        ...
                        rowRender: false,
                        tableRender: false,
                        diffDomOptions: { valueDiffing: false },
                        labels: {
                            language: {
                                url: '//cdn.datatables.net/plug-ins/2.1.7/i18n/es-MX.json',
                            },
                            placeholder: "Buscar por todos los campos",
                            perPage: `${type} por página`,
                            pageTitle: "Página {page}",
                            noRows: `No se encontraron ${type}`,
                            noResults: "No hay resultados que coincidan con tu búsqueda",
                            info: `Mostrar {start} a {end} de {rows} ${type}` // Ajustar el mensaje info
                        },

                        template: (t, e) => `<div class='${t.classes.top}'>\n    ${t.searchable ? `<div class='${t.classes.search}' style='float: left;'>\n            <input class='${t.classes.input}' placeholder='${t.labels.placeholder}' type='search' name="search" title='${t.labels.searchTitle}'${e.id ? ` aria-controls="${e.id}"` : ""} style='width: 250px; border-radius: 8px;'>\n        </div>` : ""}\n    ${t.paging && t.perPageSelect ? `<div class='${t.classes.dropdown}' style='float: right;'>\n            <label>\n                <select class='${t.classes.selector}' name="per-page"></select> ${t.labels.perPage}\n            </label>\n        </div>` : ""}\n</div>\n<div class='${t.classes.container}'${t.scrollY.length ? ` style='height: ${t.scrollY}; overflow-Y: auto;'` : ""}></div>\n<div class='${t.classes.bottom}'>\n    ${t.paging ? `<div class='${t.classes.info}'></div>` : ""}\n    <nav class='${t.classes.pagination}'></nav>\n</div>`,
